...
class EmbeddingHead(nn.Module):
    ...
    def __init__(
        ...
        if self.cls_enable:
            ...
            if self.cls_type == 'ProxyLinear':
                self.weight = nn.Parameter(F.normalize(torch.randn(num_classes, feat_dim), dim=-1))
            else:
    ...
    def reset_parameters(self) -> None:
        ...
        if self.cls_enable and self.cls_type != "ProxyLinear":
