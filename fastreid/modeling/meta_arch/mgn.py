...
#from fastreid.modeling.losses import adasp_loss, SupConLoss, MultiSimilarityLoss
...
class MGN(nn.Module):
            ...
            'loss_kwargs':
                {
                    ...
                    'adasp': {
                        ...
                        'all_epochs': cfg.SOLVER.MAX_EPOCH
                    },
                    'ep': {
    ...
    def losses(self,
        ...
        if 'CircleLoss' in loss_names:
            ...
            loss_dict['loss_circle_b1'] = pairwise_circleloss(
                b1_pool_feat,
                gt_labels,
                circle_kwargs.get('margin'),
                circle_kwargs.get('gamma')
            ) * circle_kwargs.get('scale') * 0.2

            loss_dict['loss_circle_b2'] = pairwise_circleloss(
                b2_pool_feat,
                gt_labels,
                circle_kwargs.get('margin'),
                circle_kwargs.get('gamma')
            ...
            loss_dict['loss_circle_b3'] = pairwise_circleloss(
                b3_pool_feat,
                gt_labels,
                circle_kwargs.get('margin'),
                circle_kwargs.get('gamma')
            ...
            loss_dict['loss_circle_b22'] = pairwise_circleloss(
                b22_pool_feat,
                gt_labels,
                circle_kwargs.get('margin'),
                circle_kwargs.get('gamma')
            ...
            loss_dict['loss_circle_b33'] = pairwise_circleloss(
                b33_pool_feat,
                gt_labels,
                circle_kwargs.get('margin'),
                circle_kwargs.get('gamma')
        ...
        if 'MSLoss' in loss_names:
            ...
            loss_dict['loss_ms_b1'] = loss_func(nn.functional.normalize(b1_pool_feat, dim = 1), gt_labels)*ms_kwargs.get('scale') * 0.2

            loss_dict['loss_ms_b2'] = loss_func(nn.functional.normalize(b2_pool_feat, dim = 1), gt_labels)*ms_kwargs.get('scale') * 0.2

            loss_dict['loss_ms_b3'] = loss_func(nn.functional.normalize(b3_pool_feat, dim = 1), gt_labels)*ms_kwargs.get('scale') * 0.2

            loss_dict['loss_ms_b22'] = loss_func(nn.functional.normalize(b22_pool_feat, dim = 1), gt_labels)*ms_kwargs.get('scale') * 0.2

            loss_dict['loss_ms_b33'] = loss_func(nn.functional.normalize(b33_pool_feat, dim = 1), gt_labels)*ms_kwargs.get('scale') * 0.2

        if 'SupConLoss' in loss_names:
            supcon_kwargs = self.loss_kwargs.get('supcon')
            loss_func = SupConLoss()

            loss_dict['loss_supcon_b1'] = loss_func(nn.functional.normalize(b1_pool_feat, dim = 1).unsqueeze(1), gt_labels)*supcon_kwargs.get('scale') * 0.2

            loss_dict['loss_supcon_b2'] = loss_func(nn.functional.normalize(b2_pool_feat, dim = 1).unsqueeze(1), gt_labels)*supcon_kwargs.get('scale') * 0.2

            loss_dict['loss_supcon_b3'] = loss_func(nn.functional.normalize(b3_pool_feat, dim = 1).unsqueeze(1), gt_labels)*supcon_kwargs.get('scale') * 0.2

            loss_dict['loss_supcon_b22'] = loss_func(nn.functional.normalize(b22_pool_feat, dim = 1).unsqueeze(1), gt_labels)*supcon_kwargs.get('scale') * 0.2

            loss_dict['loss_supcon_b33'] = loss_func(nn.functional.normalize(b33_pool_feat, dim = 1).unsqueeze(1), gt_labels)*supcon_kwargs.get('scale') * 0.2
        
        if 'EPLoss' in loss_names:
            ep_kwargs = self.loss_kwargs.get('ep')
            loss_func = EPHNLoss()

            loss_dict['loss_ep_b1'] = loss_func(b1_pool_feat, gt_labels)*ep_kwargs.get('scale') * 0.2

            loss_dict['loss_ep_b2'] = loss_func(b2_pool_feat, gt_labels)*ep_kwargs.get('scale') * 0.2

            loss_dict['loss_ep_b3'] = loss_func(b3_pool_feat, gt_labels)*ep_kwargs.get('scale') * 0.2

            loss_dict['loss_ep_b22'] = loss_func(b22_pool_feat, gt_labels)*ep_kwargs.get('scale') * 0.2

            loss_dict['loss_ep_b33'] = loss_func(b33_pool_feat, gt_labels)*ep_kwargs.get('scale') * 0.2
        ...
        if "AdaSPLoss" in loss_names:
            ...
            loss_dict['loss_adasp_b1'] = loss_func(b1_pool_feat, gt_labels)*adasp_kwargs.get('scale')* 0.2

            loss_dict['loss_adasp_b2'] = loss_func(b2_pool_feat, gt_labels)*adasp_kwargs.get('scale')* 0.2

            loss_dict['loss_adasp_b3'] = loss_func(b3_pool_feat, gt_labels)*adasp_kwargs.get('scale')* 0.2
            
            loss_dict['loss_adasp_b22'] = loss_func(b22_pool_feat, gt_labels)*adasp_kwargs.get('scale')* 0.2

            loss_dict['loss_adasp_b33'] = loss_func(b33_pool_feat, gt_labels)*adasp_kwargs.get('scale')* 0.2
