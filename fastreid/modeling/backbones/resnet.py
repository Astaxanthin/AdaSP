...
model_urls = {
    ...
    '152x': 'https://download.pytorch.org/models/resnet152-394f9c45.pth',
...
class ResNet(nn.Module):
    ...
    def forward(self, moco_x):
        if isinstance(moco_x, dict):
            x = moco_x['images']
        else:
            x = moco_x
        ...
        if isinstance(moco_x, dict):
            moco_dict = dict()
            moco_dict['features'] = x
            moco_dict['targets'] = moco_x['targets']
            return moco_dict
        else:
            return x
