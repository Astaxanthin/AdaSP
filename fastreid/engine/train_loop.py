...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, model, data_loader, optimizer, param_wrapper):
        ...
        self._data_loader_iter = list()
        for i in range(len(data_loader)):
            self._data_loader_iter.append(iter(data_loader[i]))
    ...
    def run_step(self):
        ...
        dl_index = self.iter%len(self.data_loader)  # switch dataloader
        if len(self.data_loader) > 1:
            data_0 = next(self._data_loader_iter[0])
            data_1 = next(self._data_loader_iter[1])
        
            data = dict()
            for k,v in data_0.items():
                if torch.is_tensor(data_0[k]):
                    if k == 'targets':
                        data_1[k] = data_1[k] + max(data_0[k]) + 1
                    data[k] = torch.cat((data_0[k],data_1[k]),0)
                elif isinstance(data_0[k],list):
                    data[k] = data_0[k]
                    data[k].extend(data_1[k])
        else:
            data = next(self._data_loader_iter[dl_index])
    ...
    def _write_metrics(self, loss_dict: Dict[str, torch.Tensor], data_time: float, neg_sim = -2., neg_var = -2.):
        """
        Args:
        ...
        with torch.cuda.stream(torch.cuda.Stream() if device.type == "cuda" else None):
            ...
            metrics_dict["neg_sim"] = neg_sim.detach().cpu().item() if neg_sim != -2 else -2.
            metrics_dict["neg_var"] = neg_var.detach().cpu().item() if neg_var != -2 else -2.
        ...
        if comm.is_main_process():
            ...
            neg_sim = np.mean([x.pop("neg_sim") for x in all_metrics_dict])
            storage.put_scalar("neg_sim", neg_sim)

            neg_var = np.mean([x.pop("neg_var") for x in all_metrics_dict])
            storage.put_scalar("neg_var", neg_var)
...
class AMPTrainer(SimpleTrainer):
    ...
    def run_step(self):
       ...
       #     print(data_0['targets'].shape)
        #    print(data_1['targets'].shape)
