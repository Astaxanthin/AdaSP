...
def default_argument_parser():
    ...
    parser.add_argument("--config-file", default="./configs/MSMT17/mgn_CE_adasp.yml", metavar="FILE", help="path to config file")
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument("--save-only", action="store_true", help="save jit model")
    parser.add_argument("--output", default='./output/', help="output file address")
    parser.add_argument("--weights", default='./logs/veri/sbs_R50-ibn/model_best.pth', help="model weights address")

    # PyTorch still may leave orphan processes in multi-gpu training.
    # Therefore we use a deterministic way to obtain port,
    # so that users are aware of orphan processes by seeing the port occupied.
    port = 2 ** 15 + 2 ** 14 + hash(os.getuid() if sys.platform != "win32" else 1) % 2 ** 14
    parser.add_argument("--dist-url", default="auto")  #"tcp://127.0.0.1:{}".format(port))
    #parser.add_argument("tcp://127.0.0.1:{}".format(port))
    parser.add_argument(
        "opts",
...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        for i in range(len(data_loader)):
            num_classes = min(num_classes, data_loader[i].dataset.num_classes)
            num_data = min(num_data, len(data_loader[i].dataset))


        cfg = self.auto_scale_hyperparams(cfg, num_classes)
        ...
        #self.iters_per_epoch = num_data*len(data_loader) // cfg.SOLVER.IMS_PER_BATCH
        self.iters_per_epoch = num_data // cfg.SOLVER.IMS_PER_BATCH
