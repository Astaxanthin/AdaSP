...
class LayerFreeze(HookBase):
    ...
    def freeze_specific_layer(self):
        ...
        sub_key, sub_model = model_children[0]
        flag = False
        for layer in self.freeze_layers:
            if not hasattr(self.model, layer) and not hasattr(sub_model, layer):
        ...
        if flag:
            ...
            self.is_frozen = False
            freeze_layers = ", ".join(self.freeze_layers)
            self._logger.info(f'Freeze layer group "{freeze_layers}" training failed !')
    ...
    def open_all_layer(self):
        ...
        if flag:
            ...
            freeze_layers = ", ".join(self.freeze_layers)
            self._logger.info(f'Open layer group "{freeze_layers}" failed !')
