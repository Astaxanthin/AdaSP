...
def _train_loader_from_config(cfg, *, train_set=None, transforms=None, sampler=None, **kwargs):
    ...
    if train_set is None:
        ...
        for d in cfg.DATASETS.NAMES:
            ...
            train_set.append(CommDataset(data.train, transforms, relabel=True))


    mini_batch_size = list()
    if sampler is None:
        ...
        for d in cfg.DATASETS.NAMES:
            if d == "BDD":
                mini_batch_size.append(cfg.SOLVER.IMS_PER_BATCH)
            else:
                mini_batch_size.append(cfg.SOLVER.IMS_PER_BATCH)
        ...
        sampler = list()
        for i in range(len(cfg.DATASETS.NAMES)):
            if sampler_name == "TrainingSampler":
                sampler.append(samplers.TrainingSampler(len(train_set[i])))
            elif sampler_name == "NaiveIdentitySampler":
                sampler.append(samplers.NaiveIdentitySampler(train_set[i].img_items, mini_batch_size[i] // comm.get_world_size(), num_instance))
            elif sampler_name == "BalancedIdentitySampler":
                sampler.append(samplers.BalancedIdentitySampler(train_set[i].img_items, mini_batch_size[i]// comm.get_world_size(), num_instance))
            elif sampler_name == "SetReWeightSampler":
                set_weight = cfg.DATALOADER.SET_WEIGHT
                sampler.append(samplers.SetReWeightSampler(train_set[i].img_items, mini_batch_size[i]// comm.get_world_size(), num_instance, set_weight))
            elif sampler_name == "ImbalancedDatasetSampler":
                sampler.append(samplers.ImbalancedDatasetSampler(train_set[i].img_items))
            else:
    ...
    return {
        "train_set": train_set,
        "sampler": sampler,
        "total_batch_size": mini_batch_size,
...
def build_reid_train_loader(
    ...
    for i in range(len(train_set)):
        batch_sampler = torch.utils.data.sampler.BatchSampler(sampler[i], total_batch_size[i] // comm.get_world_size(), True)

        train_loader.append(DataLoaderX(
            comm.get_local_rank(),
            dataset=train_set[i],
